...
import {
  parseSinunaAuthenticateResponse,
  SinunaStateAttributor,
} from "./utils/SinunaResponseParsers";
...
export default new (class SinunaRequestHandler implements AuthRequestHandler {
  ...
  async LoginRequest(context: Context): Promise<HttpResponse> {
    ...
    const REDIRECT_URI = Runtime.getAppUrl(
  ...
  async AuthenticateResponse(context: Context): Promise<HttpResponse> {
    const authenticateResponse = parseSinunaAuthenticateResponse(
      context.request.query
    );
    const appContextObj = authenticateResponse.appContextObj;
    const redirectUrl = prepareLoginRedirectUrl(
      appContextObj.redirectUrl,
      authenticateResponse.loginCode,
      ...
      const response = await axios.post(
        ...
        }).toString(),
        {
          headers: {
            ...
            Authorization:
              "Basic " + generateBase64Hash(`${CLIENT_ID}:${CLIENT_SECRET}`),
          },
        }
      );
  ...
  async LogoutRequest(context: Context): Promise<HttpResponse> {
    const appContext = parseAppContext(context, this.identityProviderIdent);
    const LOGOUT_CALLBACK_REDIRECT_URI = Runtime.getAppUrl(
  ...
  async LogoutResponse(context: Context): Promise<HttpResponse> {
    const appContext = parseAppContext(context, this.identityProviderIdent);
    const redirectUrl = prepareLogoutRedirectUrl(
  ...
  async UserInfoRequest(context: Context): Promise<HttpResponse> {
    ...
    try {
      const response = await axios.get(
        `https://login.iam.qa.sinuna.fi/oxauth/restv1/userinfo`,
        {
          headers: {
  ...
  async AuthorizeRequest(context: Context): Promise<HttpResponse> {
    ...
    const appName = parseAppContext(context, this.identityProviderIdent).object
      .appName;
