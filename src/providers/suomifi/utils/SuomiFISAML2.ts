...
export const getSuomiFISAML2Client = async function (): Promise<typeof SAML> {
  if (!suomiSaml) {
    ...
    suomiSaml = new SAML({
      ...
      cert: async function (
        callback: (err: any, certs?: Array<string>) => void
      ) {
        try {
          ...
          const certs = [
            xmljsDoc.EntityDescriptor?.Signature?.[0]?.KeyInfo?.[0]
              ?.X509Data?.[0]?.X509Certificate?.[0]?._,
            xmljsDoc?.EntityDescriptor?.IDPSSODescriptor?.[0]
              ?.KeyDescriptor?.[0]?.KeyInfo?.[0]?.X509Data?.[0]
              ?.X509Certificate?.[0]?._,
            xmljsDoc?.EntityDescriptor?.IDPSSODescriptor?.[0]
              ?.KeyDescriptor?.[1]?.KeyInfo?.[0]?.X509Data?.[0]
              ?.X509Certificate?.[0]?._,
          ]
